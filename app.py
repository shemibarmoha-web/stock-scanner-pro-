...
if stock_symbol:
    ...
    if "נרות יפניים" in analysis_page:
        ...
        st.write("מעקב אחר התנהגות המחירים יום אחר יום לזיהוי עוצמת הקונים והמוכרים:")
        
        # טבלת נרות מסודרת שנראית כמו סקריפט מסחר
        candles_df = pd.DataFrame({
            'יום מסחר': ['יום שלישי', 'יום רביעי', 'יום חמישי', 'היום (נוכחי)'],
            'שער פתיחה (₪)': [82.0, 83.5, 83.0, 84.2],
            'שער גבוה (₪)': [83.8, 84.5, 84.0, 86.2],
            'שער נמוך (₪)': [81.5, 83.0, 82.5, 83.9],
            'שער סגירה (₪)': [83.5, 83.0, 84.2, 86.15],
            'מגמת נר': ['🟢 ירוק (עולה)', '🔴 אדום (יורד)', '🟢 ירוק (עולה)', '🟢 בליעה שורית חזקה']
        })
        st.dataframe(candles_df, use_container_width=True)
        
        st.success("""
        💡 **ניתוח מצב נוכחי:** 
        * הנר האחרון מראה **פריצה עוצמתית** כלפי מעלה עם נפח מסחר גבוה.
        * התבנית מעידה על שליטה מלאה של הקונים ויציאה ממתחם ההתנגדות הקצר.
        """)
