...
if stock_symbol:
    ...
    if "גרף נרות יפניים" in analysis_page:
        ...
        # נפח מסחר (תוקן ללא פרמטרים מיותרים)
        ...
        if timeframe == "חודש אחרון":
            ...
            * **למי שרוצה לקנות:** זהו **לא** אזור אידיאלי לקנייה אגרסיבית. מכיוון שהמחיר מדשדש בגבהים, החכם יהיה **להמתין בסבלנות** לנר היפוך חזק כלפי מעלה, או לחלופין לירידה קלה שתבחן את אזור התמיכה הקרוב כדי להיכנס במחיר זול יותר.
            * **למי שרוצה למכור / מחזיק במניה:** אין סיבה לרוץ ולמכור רק כי יש ימים אדומים קטנים כל עוד נפחי המסחר נמוכים (אין בריחת כסף גדול). אפשר להגדיר פקודת "סטופ-לוס" (הגנה) מתחת לאזור התמיכה האחרון כדי להגן על הרווחים שנצברו.
            """)
        elif timeframe == "3 חודשים":
            st.markdown("""
            ...
            **2. 🧭 מה הקונים והמוכרים אמורים לעשות במצב כזה?**
            * **למי שרוצה לקנות:** המגמה הראשית היא חיובית (שורית), ולכן כיוון המסחר הנכון הוא חיפוש הזדמנויות קנייה. ההמלצה המעשית היא **לקנות בירידות קטנות בתוך המגמה העולה** או להצטרף לפריצה של אזור התנגדות עם נפח מסחר גבוה. להיזהר לא לרדוף אחרי המניה ביום של זינוק חד מדי.
            ...
            * **למי שרוצה למכור:** זהו הזמן לבצע הערכת מצב פונדמנטלית וטכנית רחבה: האם מחיר המניה הגיע ליעד הרווח שהוגדר מראש? אם כן, מומלץ לממש חלק מהרווחים ולהשאיר את היתרה להמשך צמיחה.
            """)
        else:
            st.markdown("""
            ...
            **2. 🧭 מה הקונים והמוכרים אמורים לעשות במצב כזה?**
            * **למי שרוצה לקנות/למכור:** מבט היסטורי נועד בעיקר כדי להבין אם המניה נסחרת כרגע ב"תמחור יתר" קיצוני או ב"הזדמנות ערך" היסטורית. אם המחיר נמצא סביב רמות שפל רב-שנתיות חזקות, משקיעי ערך נוטים לראות בכך איתות הצטיידות לטווח הארוך מאוד. אם המחיר נמצא בתקרה היסטורית קשה, מומלץ להקטין חשיפה.
            """)
