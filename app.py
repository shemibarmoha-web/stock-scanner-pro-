...
if stock_symbol:
    ...
    if "גרף נרות יפניים" in analysis_page:
        ...
        * **פעילות נפח המסחר:** בחינת עוצמת הפעילות מאחורי תנועות המחיר בטוח הנבחר.
        """)
        ...
        # --- ניתוח טכני מקצועי וטהור (ללא המלצות) ---
        st.markdown("---")
        st.markdown("### 🎓 ניתוח טכני מקצועי: קריאת מומנטום ומבנה מגמה")
        st.markdown("""
        #### 1. פיענוח מבנה הקווים בגרף:
        * **שער המניה (קו כחול):** משקף את המחיר בשוטף.
        * **ממוצע נע מהיר (MA 7 - קו ירוק):** מייצג את רגש השוק לטווח הקצר מאוד (7 ימי מסחר).
        * **ממוצע נע קצר (MA 20 - קו כתום מקוטע):** מייצג את המגמה העוקבת לטווח הבינוני (20 ימי מסחר / כחודש).

        #### 2. זיהוי מצבי מומנטום:
        * **מומנטום חיובי:** מתקיים כאשר קו המחיר (הכחול) והממוצע המהיר (הירוק) ממוקמים **מעל** הממוצע הקצר (הכתום), דבר המצביע על שליטת קונים ועליית ערך עקבית בטוח הנצפה.
        * **מומנטום שלילי / דעיכה:** מתקיים כאשר הממוצע המהיר (הירוק) חוצה את הממוצע הקצר (הכתום) **כלפי מטה**, או לחלופין כאשר שער המניה יורד מתחת לשניהם, מה שמעיד על לחץ מוכרים ומעבר למגמה יורדת.
        """)
