...
if stock_symbol:
    ...
    if "גרף נרות יפניים" in analysis_page:
        ...
        st.markdown("---")
        st.markdown("### 🎓 ניתוח טכני: תמונת מצב מומנטום")
        st.markdown("""
        * **מצב המחיר והנרות:** מעקב אחר התפלגות השערים והפערים בין שערי הפתיחה לסגירה לאורך זמן.
        * **פעילות נפח המסחר:** בחינת עוצמת הפעילות מאחורי תנועות המחיר בטווח הנבחר.
        """)
        ...
        # --- הסבר מקצועי וטהור על המשמעות של ממוצעים נעוצים ---
        st.markdown("---")
        st.markdown("### 🎓 ניתוח טכני: המשמעות של ממוצעים נעוצים (MA 7 מול MA 20)")
        st.markdown("""
        #### 1. מהו ממוצע נע ובמה הוא עוזר?
        מטרת הממוצע הנע היא לסנן את רעשי השקפים היומיים והזיגזגים הקטנים, ולהציג את הכיוון האמיתי של המחיר לאורך זמן באמצעות חישוב ממוצע שערים.

        #### 2. המשמעות של ממוצע נע מהיר (7 ימים - קו ירוק):
        * **אופן החישוב:** ממוצע שערים של שבוע המסחר האחרון בלבד.
        * **משמעות טכנית:** בשל טווח הזמן הקצר מאוד, קו זה רגיש לכל שינוי חד ומייצג את **המומנטום המיידי** בשוק ואת התגובה המהירה ביותר להתפתחויות האחרונות במחיר.

        #### 3. המשמעות של ממוצע נע קצר (20 ימים - קו כתום מקוטע):
        * **אופן החישוב:** ממוצע שערים של 20 ימי מסחר (כחודש מסחר מלא).
        * **משמעות טכנית:** קו זה רגיש פחות לתנודות קצרצרות, ולכן משקף את **המגמה הבסיסית והיציבה יותר** לאורך החודש האחרון.

        #### 4. ניתוח המצבים בגרף:
        * **מומנטום חיובי:** מתקיים כאשר קו המחיר והממוצע המהיר (7 ימים) נמצאים מעל הממוצע הקצר (20 ימים), מה שמעיד על לחץ ביקושים שדוחף את השערים מעל הממוצע החודשי.
        * **מומנטום שלילי / דעיכה:** מתקיים כאשר הממוצע המהיר יורד מתחת לממוצע הקצר או שהמחיר יורד מתחת לשניהם, מה שמעיד על היחלשות בביקושים המיידיים ביחס למגמת החודש האחרון.
        """)
