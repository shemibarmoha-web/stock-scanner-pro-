...
if stock_symbol:
    ...
    if "גרף נרות יפניים" in analysis_page:
        ...
        fig.update_yaxes(side="right", row=2, col=1, automargin+True if 'automargin' in locals() else True)
        ...
        if timeframe == "חודש אחרון":
            ...
            * **המבנה הבסיסי למתחילים:** כל "נר" בגרף מייצר סיפור של יום מסחר שלם. נר ירוק אומר שהמחיר סיים גבוה מכפי שהתחיל (הקונים ניצחו היום), ונר אדום אומר שהמחיר סיים נמוך מכפי שהתחיל (המוכרים ניצחו). הפתילים (הקווים הדקים שיוצאים מהנר) מראים את שיא הגובה והשפל שאליהם הגיע המחיר במהלך אותו יום לפני שנסגר.
            ...
            * **ניתוח עומק של נפח המסחר (העמודות למטה):** שים לב לעמודות הנפח בתחתית הגרף. בימים האדומים (שבהם המחיר ירד), העמודות קצרות ונמוכות משמעותית בהשוואה לימים הירוקים. מבחינה מקצועית, זה סימן מעודד מאוד: המשמעות היא שאין פאניקה בשוק ואין גופים גדולים שמוכרים בהستاهלות. הירידות הקלות הן תוצאה של מימושי רווח טבעיים של סוחרים קטנים, ולא בריחה המונית של כסף חכם.
            """)
        elif timeframe == "3 חודשים":
            st.markdown("""
            ...
            * **ניתוח עומק ופסיכולוגיית משקיעים:** רצף הנרות הירוקים הארוכים בשלבים המוקדמים של התקופה מלמד על כניסה של כסף מוסדי (גופים גדולים כמו קרנות פנסיה או בתי השקעות) שדוחפים את המחיר כלפי מעלה בעוצמה. האזורים התחתונים שמהם החל הזינוק מתפקדים כעת כ"רמת תמיכה" קשיחה – כלומר, רמה שבה הקונים יחזרו לקנות אוטומטית כי הם מזהים בה מחיר אטרקטיבי.
            """)
        elif timeframe == "שנה אחרונה":
            st.markdown("""
            ...
            * **מה קורה כרגע במגמה?** הגרף חושף את נקודות המפנה המרכזיות בשנה החולפת. ניתן לראות כיצד המניה נחלשה בתקופות מסוימות (מחזורי ירידה ממושכים) אך ידעה לבנות את עצמה מחדש, לפרוץ רמות התנגדות קשות, ולייצר תשואה מצטברת מרשימה למשקיעים לטווח ארוך.
            * **ניתוח עומק מוסדי:** בטווח כזה, נפחי המסחר הגבוהים המלווים בנרות ארוכים מסמנים את האירועים הכלכליים המשמעותיים ביותר (כמו פרסום דו"חות כספיים שנתיים או עסקאות ענק), שבהם השוק קבע מחדש את שוויה הכלכלי האמיתי של החברה.
            """)
        else:
            st.markdown("""
            ...
            * **ניתוח עומק מרכזי:** הסתכלות על כל ההיסטוריה מאפשרת לזהות את "תקרות הזכוכית" ההיסטוריות של המניה ואת רמות השפל העמוקות ביותר שלה. זהו הכלי המרכזי שמשקיעי ערך גדולים משתמשים בו כדי להבין האם המחיר הנוכחי נמצא בבועה יחסית לעבר, או שמדובר בהזדמנות קנייה היסטורית ארוכת טווח.
            """)
